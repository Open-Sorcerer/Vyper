...
export default async function createFrontend() {
    ...
    switch (results.framework) {
        case "NextJS":
            ...
            // execute npm install
            // execute npm run dev
            exec(`git clone https://github.com/Open-Sorcerer/Vyper-Next ${results2.projectName}`, (err, stdout, stderr) => {
                if (err) {
        ...
        case "ReactJS":
            ...
            exec(`git clone https://github.com/Open-Sorcerer/Vyper-Vite ${results3.projectName}`, (err, stdout, stderr) => {
                if (err) {
                    console.log(chalk.redBright(err));
                    return;
                }
